...
class Textbar
{
	SDL_Rect text_bar;
	void draw_textarea(){
		grad_square(0, 0,SCRX,38,status_down_color, status_up_color);
	}
	public:
		bool mouse_over;
		Textbar()
		{
			text_bar.x=20;
			text_bar.y=6;
			font = TTF_OpenFont( "resources//crdr.otf", 26 );
			change_text("ברוכים הבאים לקורידור");
		}
...
class Menu
{
	...
	public:
		...
		Uint16 click_event(int y,Textbar *actiontext)
		{
			...
			switch(y)
			{
				...
				case 3:
					if (fullscreen)
						actiontext->change_text("אי אפשר לפתוח קישורים במסך מלא");
					else
						system ("firefox resources/howto-he.html");
				...
				case 5:
                     return 4;
				case 6:
					actiontext->change_text("אפשרות זו אינה זמינה כעת");
					break;
				case 7:
					return 1;
			}
		...
		void move_event(int y,Textbar *actiontext)
		{
			...
			if (y!=lastmove)
				...
				{
					case 1:
						actiontext->change_text("התחל משחק חדש");
						break;
					case 2:
                        if (fullscreen)
						   actiontext->change_text("צא ממסך מלא");
		                else
		                    actiontext->change_text("היכנס למסך מלא");
						break;
					case 3:
						actiontext->change_text("הצג את הוראות המשחק");
						break;
					case 4:
						actiontext->change_text("כל הזכויות שמורות לאור דבורי. המשחק מופץ תחת הרישיון הציבורי הכללי של גנו");
						break;
					case 5:
						actiontext->change_text("ביטול המהלך האחרון");
						break;
					case 6:
						actiontext->change_text("שחק קורידור נגד המחשב");
						break;
					case 7:
						actiontext->change_text("צא מהמשחק");
						break;
		...
		void draw_button(int y,std::string button_text){
            button_text=reverse(button_text);
	...
	void draw_menu(){
		draw_button(40,"משחק חדש");
		draw_button(120,"מסך מלא");
		draw_button(200,"כיצד לשחק");
		draw_button(280,"אודות");
		draw_button(360,"בטל מהלך");
		draw_button(440,"משחק נגד המחשב");
		draw_button(520,"יציאה");
	}
};
class Game
{
			...
			if(!game_end)
			{
				if (turn)
				{
					if (crimson->onplayer(x,y)&& !(crimson->mouse_over))
					{
						...
						if (crimson->wselected!=10)
						{
							actiontext->change_text("לחץ כאן כדי לבחור את החייל הכהה");
					...
					if (fawn->onplayer(x,y)&& !(fawn->mouse_over))
					{
						...
						{
						actiontext->change_text("לחץ כאן כדי לבחור את החייל הבהיר");
			...
			if (turn)
			{
				...
				else 
				{
					if(crimson->wselected==10)
					{
						...
						{
								game_end=true;
								actiontext->change_text("השחקן הראשון ניצח עם החייל הכהה");
						}
						else if (possible !=0)
							change_turn();
					}
			...
			else
			{
				...
				else 
				{
					if(fawn->wselected==10)
					{
						...
						{
								game_end=true;
								actiontext->change_text("השחקן השני ניצח עם החייל הבהיר");
						}
						else if (possible !=0)
							change_turn();
					}
			...
			switch (event.type) 
			{
				case SDL_MOUSEBUTTONUP:
					...
					else 
					{
						if (!main_game->game_end)
							if (main_game->turn)
								actiontext->change_text("עכשיו תורו של השחקן הראשון עם החייל הכהה");
							else
								actiontext->change_text("עכשיו תורו של השחקן השני עם החייל הבהיר");
						else
							if (main_game->turn)
								actiontext->change_text("החייל הכהה ניצח. אפשר להתחיל משחק חדש");
							else 
								actiontext->change_text("החייל הבהיר ניצח. אפשר להתחיל משחק חדש");
					}
				...
				case SDL_MOUSEMOTION:
					...
					else 
					{
						main_menu->notover();
						if (!actiontext->mouse_over)
							actiontext->change_text("קורידור");
